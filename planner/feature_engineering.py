...
def create_grid_graph(min_lat=6.0, max_lat=35.0, min_lng=68.0, max_lng=98.0, grid_size=0.5):
    ...
    for node in G.nodes():
        ...
        for other_node in G.nodes():
            if node != other_node:
                ...
                # Connect only to immediate neighbors (horizontally, vertically, diagonally)
                if (abs(lat1 - lat2) <= grid_size and abs(lng1 - lng2) <= grid_size):
...
def add_obstacle_features(G, earthquake_df, tsunami_df):
    ...
    for _, event in tsunami_df.iterrows():
    ...
    for _, event in earthquake_df.iterrows():
        if pd.isna(event['Latitude']) or pd.isna(event['Longitude']):
            continue
...
def generate_training_routes(G, num_routes=1000):
...
def save_processed_data(X, y, scaler, graph, routes, output_dir):
    ...
    graph_data = {
        'nodes': {str(n): {'lat': G.nodes[n]['lat'], 'lng': G.nodes[n]['lng']} for n in G.nodes()},
        'edges': [{
            'source': str(u),
            'target': str(v),
            ...
            'flood_risk': G[u][v]['flood_risk'],
            'debris_risk': G[u][v]['debris_risk']
        } for u, v in G.edges()]
    }
    ...
    with open(os.path.join(output_dir, 'sample_routes.json'), 'w') as f:
        json.dump(routes[:100], f)  # Save first 100 routes

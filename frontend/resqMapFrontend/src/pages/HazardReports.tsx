    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {mockHazards.map((hazard) => (
          <div key={hazard.id} className="bg-white rounded-lg shadow-md p-4 border border-neutral-200">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center">
                ...
                <h3 className="font-semibold">{hazard.location.name}</h3>
              </div>
              <Badge 
                variant={
                  hazard.severity === 'high' ? 'danger' : 
                  hazard.severity === 'medium' ? 'warning' : 
                  'neutral'
                }
                size="sm"
              >
            ...
            <div className="text-sm text-neutral-500">
              <div>Lat: {hazard.location.lat}</div>
              <div>Lng: {hazard.location.lng}</div>
            </div>
          </div>
        ))}
      </div>
